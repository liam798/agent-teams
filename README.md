...
```bash
agent-teams --json doctor
teams --json doctor
```

日常建议用短命令 `teams`；`agent-teams` 保留用于兼容脚本和文档。
...
agent-teams teams create my-team --member 审查员:claude --member 架构师:codex --member 测试:gemini
```
...
```bash
# 单任务
agent-teams tasks add my-team "审查 auth 模块安全性"

# 带描述与依赖
agent-teams tasks add my-team "实现登录 API" --desc "REST API" --dep task_xxx
```
...
```bash
agent-teams tasks add-batch my-team --file tasks.json
```

### 3. 查看任务

```bash
agent-teams tasks list my-team
```

### 4. 查看团队成员

```bash
agent-teams members list my-team
```
...
```bash
agent-teams members run my-team
```

工作目录默认为当前目录，可通过 `--cwd` 指定：

```bash
agent-teams members run my-team --cwd /path/to/project
```
...
```bash
agent-teams members spawn my-team 安全审查员 claude "审查 src/auth/ 的安全与性能，并报告发现。"
```
...
```bash
agent-teams teams delete my-team
```

旧命令仍保持兼容：`create`、`add-task`、`add-tasks`、`tasks <团队名>`、`members <团队名>`、`run`、`spawn`、`cleanup`、`platforms`、`ui`、`serve`、`install-skill`、`list-skills`、`skill-path` 都可以继续使用。

## CLI 契约

面向 Agent 和脚本，推荐优先使用资源型子命令，并在需要解析输出时加 `--json`：

```bash
agent-teams --json doctor
agent-teams --json teams list
agent-teams --json teams resolve my-team
agent-teams --json teams get my-team
agent-teams --json tasks list my-team --status pending --limit 20
agent-teams --json tasks add my-team "实现登录 API" --desc "REST API" --dry-run
agent-teams --json members list my-team
agent-teams --json messages list my-team --limit 20
...
```bash
teams list
teams create my-team --member 审查员:codex
teams tasks my-team
teams run my-team
```
...
- `doctor`：检查安装、配置、存储目录、平台可用性；本工具不需要网络认证。
- `init`：写入用户级配置，例如 `agent-teams init --storage /path/to/storage`。
- `teams`：`list`、`get`、`resolve`、`create`、`delete`。
- `tasks`：`list`、`get`、`add`、`add-batch`、`claimable`、`set-status`。
- `members`：`list`、`spawn`、`run`、`stop`。
- `messages`：`list`、`send`。
- `skills`：`list`、`path`、`install`。
- `server start`：启动 Web UI。
- `request get`：只读 raw 逃生口，支持 `/storage`、`/platforms`、`/teams`、`/teams/<team>`、`/tasks/<team>`、`/messages/<team>`。

JSON 输出策略：

- `--json` 下，成功结果只向 stdout 输出 JSON；诊断和进度输出写入 stderr。
- 写命令返回 `{ "ok": true, ... }`；读命令返回对象或 `{ "items": [...], "count": n }`。
- 错误在 stderr 输出 `{ "ok": false, "error": { "code": "...", "message": "...", "details": {} } }`，不会包含令牌或凭据。
- 支持写入的命令优先提供 `--dry-run` 预览：`teams create`、`teams delete`、`tasks add`、`tasks add-batch`、`tasks set-status`、`messages send`、`skills install`。
...
**Codex：**
```bash
agent-teams skills install codex
```
...
**其他平台：**
未来可能支持：
- `agent-teams skills install claude` - 安装到 Claude Code
- `agent-teams skills install gemini` - 安装到 Gemini CLI

### 查看技能安装路径

```bash
agent-teams skills path codex
```
